...
import "@/lib/css/Designation.css"
import MemberSearch from "@/components/agreement/MemberSearch";
import { getAccountsByIds, getAgreementsByIds, getMembershipAgreements, createGPODesignateChange, getRetryRecords, UpdateGPODesignateChange, fetchRecords, getAgreementDetailsByIds, getAccountById, queryDesignatedContractsByMember, updateAccountContract, updateAccount, createMember, getActiveGPOAgreements, getCFAMembersByAgreementIds } from "@/lib/api/services/member.service";
import { getAgreementsIds, getUserIdFromToken, getAgreementsByIdsDesignation } from "@/lib/api/services/agreement.service";
import { queryGetAgreementDetails } from "@/lib/api/services/ali.service";
...
const handleGPOUndesignate = async (memberId, designationChangeId) => {
  try {
    const contracts = await queryDesignatedContractsByMember(token, memberId);
...
console.log("All designated contracts updated with end date", formattedDate);
    const account = await getAccountById(token, memberId);
...
const designate = await getAccountById(token, r.memberId);
